...
<div class="center">
		<?php
				   ...
				   $res = $bdd->prepare('SELECT * FROM ressource,dvd WHERE IDR=IDD and IDR='.$_GET['article_id']);
				   $res->execute();
                   $info = $res->fetch();
						...
						echo '<br>';
						echo '<p>Duree: <span class="titre"><input type="text" name="r" id="r" value='.$info['Duree'].'></span></p>';
						echo '<br>';
						echo '<p>by<span class="green-text">'.$info['Acteur'].'</span></p>';
						echo '<br>';
						...
						echo '<br>';
						echo '<p><span class="titre">Editeur: '.$info['Realisateur'].'</span></p>';
						...
						try
							...
							{
							 echo '<input type="button" class="ButtonNondisponible" name="action" value="NonDisponible"/>';
							}else{
	   ...
	   $bd_commentaire = $bdd->prepare('select * from commentaire where idR='.$_GET['article_id']);
...
<?php $i=0;
		...
		if($result_commentaire = $bd_commentaire->fetch()){//ignorer  les warning 
				...
				while($result_commentaire = $bd_commentaire->fetch())//obtenir  tous les données de commentaire
				{
					...
					echo "Pseudo: <a href='pseudo.php?commentaire_id=$result_commentaire[idU]'>$result_commentaire[idU]</a>";
					echo "</td>";					
					echo "<td class='cell2'>";
		...
		<form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" id="commentaire ">				
			 ...
			 <p>
					<label>
				<input type="submit" name="button" id="button" value="Réagir " />
				</label>	
			 </p>
